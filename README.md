...
###### 万象方案从6.0开始在正式发布的时候采用了分包模式，用户可以自己选择想用的辅助码类型下载对应的包，语法模型跟着额外下载
...
**功能3**  通过 拼音状态下`〔部件组字〕`来引导拆字模式 举例 `震`  假设你不认识，你可以通过`雨和辰` 来合并输入，拼音状态输入``` ` ```后，继续输入其它字符字母``` ` ```会消失如下图，输入 `yu if` 即雨 辰，结果出现了我们要的震字，且给出了辅助码 `y` 和  `i`  ，`y`是雨的声母`y`，`i`是辰的声母`ch`
